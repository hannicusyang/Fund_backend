# routes/stock_market_overview.py
...
def serialize_date(obj):
    """Helper to ensure date objects are serialized correctly."""
...
def find_latest_record_up_to_date(query_base, date_column, target_date_obj):
    """
    Finds the latest record up to the given target date.
    Returns the record object if found, otherwise None.
    """
    if target_date_obj is None:
        return None
    # Query for the record with the maximum date that is <= target_date_obj
...
def get_sse_summary():
    """Get SSE Summary data (latest or by date)."""
    ...
    try:
        ...
        if parsed_date:
            ...
            record = StockSSESummary.query.filter_by(trade_date=parsed_date).first()
            # If not found, find the latest record up to the parsed_date
            if not record:
                logger.info(f"SSE Summary for exact date {parsed_date} not found in DB, looking for latest available.")
            ...
            record = StockSSESummary.query.order_by(desc(StockSSESummary.trade_date)).first()

        if not record:
            logger.info(f"SSE Summary for date {parsed_date} not found in DB after checking latest available.")
        ...
        if parsed_date and actual_returned_date != parsed_date:
             logger.info(f"Requested SSE Summary for {parsed_date}, returning latest available for {actual_returned_date}.")
